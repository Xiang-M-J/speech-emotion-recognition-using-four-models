...
class RNNNet(nn.Module):
    def __init__(self, feature_dim=39, drop_rate=0.2, num_class=7):  # input [batch_size, time_step, feature_dim]
        ...
        self.rnn = nn.RNN(input_size=feature_dim, hidden_size=256, batch_first=True, num_layers=1, bidirectional=True)
    ...
    def forward(self, x):
        x = x.transpose(1, 2)
        self.rnn.flatten_parameters()  # 调用flatten_parameters让parameter的数据存放成连续的块，提高内存的利用率和效率
        RNN_out, _ = self.rnn(x)  # if bidirectional=True，输出将包含序列中每个时间步的正向和反向隐藏状态的串联
        # 添加pool
